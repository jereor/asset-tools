...
namespace
{
    ...
    std::filesystem::path ResolvePath(std::string_view pathString, ToolConfig config)
    {
...
core::ToolResult ValidateMode::Run(const std::vector<std::string>& args)
{
    ...
    std::filesystem::path path = ResolvePath(args[0], m_config);
    if (!std::filesystem::exists(path))
    {
        toolResult.AddError("File or directory not found: {}", path.string());
        toolResult.exitCode = core::ExitCode::FileNotFound;
        return toolResult;
    }

    // -- Handle single file vs directory, and store asset paths --
    ...
    if (std::filesystem::is_regular_file(path))
    {
        ...
        if (!file)
        {
            toolResult.AddError("Failed to find asset file: {}", path.string());
            toolResult.exitCode = core::ExitCode::FileNotFound;
            return toolResult;
        }
    ...
    else
    {
        ...
        toolResult.AddInfo("Scanned asset directory: {}", path.string());
    }
    ...
    for (const auto& assetPath : assetPaths)
    {
        ...
        if (assetType == core::AssetType::Texture)
        {
            ...
            core::TextureAsset textureAsset = std::get<core::TextureAsset>(asset);
            results = validator.Validate(textureAsset);
        }

        // TODO: Validate audio asset with new AssetValidator 
